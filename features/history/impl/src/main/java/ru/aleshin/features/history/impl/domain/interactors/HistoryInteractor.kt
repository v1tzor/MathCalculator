...
*/
package ru.aleshin.features.history.impl.domain.interactors
...
internal interface HistoryInteractor {
    ...
    suspend fun fetchAllHistory(): DomainResult<HistoryFailures, List<CalculateHistory>>
    suspend fun deleteHistoryItem(item: CalculateHistory): UnitDomainResult<HistoryFailures>

    class Base @Inject constructor(
        private val repository: CalculatorHistoryRepository,
        private val eitherWrapper: HistoryEitherWrapper,
    ) : HistoryInteractor {

        override suspend fun fetchAllHistory() = eitherWrapper.wrap {
            repository.fetchAllHistory()
        }

        override suspend fun deleteHistoryItem(item: CalculateHistory) = eitherWrapper.wrap {
